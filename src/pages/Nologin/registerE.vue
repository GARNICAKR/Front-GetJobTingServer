  ...
  <div class="container-titleRE q-mt-md">
    <div class="row justify-between">
      ...
      <div class="col flex items-center body">
        ...
        <strong class="border size">Registrar Empleado</strong>
        <strong class="wave size">Registrar Empleado</strong>
      </div>
    </div>
  </div>
    ...
    <q-form class="" ref="registerEForm" @submit="CreateUser">
      <div class="row justify-between q-mt-md">
        ...
        <div class="col-3 q-mr-xs">
          <strong class="q-ml-md text-registerE">Telefono:</strong>
          <q-input
            filled
            class="q-px-sm"
            input-class="text-black"
            ...
            :rules="[
              (val) => (val !== null && val !== '') || 'Ingresa tu Telefono',
              (val) => val.length > 13 || 'Ingresa un telefono valido ',
            ]"
            maxlength="14"
          >
            <template v-slot:append>
              <q-icon class="text-primary" name="fa-solid fa-phone" />
            </template>
          </q-input>
        </div>
      </div>
      <div class="row justify-around q-mt-md">
        <div class="col-3 q-mr-xs q-ml-xs">
          <strong class="q-ml-md text-registerE">Nombres:</strong>
          <q-input
            filled
            class="q-px-sm"
            input-class="text-black"
        ...
        <div class="col-3 q-mr-xs">
          <strong class="q-ml-md text-registerE">Apellidos:</strong>
          <q-input
            filled
            class="q-px-sm"
            input-class="text-black"
            ...
            lazy-rules
            :rules="[
              (val) => (val !== null && val !== '') || 'Ingresa tus Apellidos',
            ]"
          >
            <template v-slot:append>
              <q-icon class="text-primary" name="fa-solid fa-circle-user" />
      ...
      <div class="row justify-around q-mt-md">
        <div class="col-5 col-md-auto">
          <strong class="q-ml-md text-registerE">Curriculum:</strong>
          <q-uploader
        ...
        <div class="col-3 self-center text-registerE">
          ...
          <q-select
            ...
            lazy-rules
            :rules="[
              (val) => (val && val.length > 0) || 'Selecciona un Sector',
            ]"
          >
            <template v-slot:append>
              <q-icon class="text-primary" name="fa-solid fa-hospital-user" />
            </template>
          </q-select>
          <q-btn
        ...
        <div class="col-5 col-md-auto">
          <strong class="q-ml-md text-registerE">Foto de Perfil:</strong>
          <q-uploader
            style="width: 350px; height: 270px"
            accept=".jpg, image/*"
            :auto-upload="false"
...
function formatPhoneNumber() {
  if (phone_number.value.length < 15) {
    phone_number.value = phone_number.value.replace(/\D/g, ""); // Elimina caracteres no numéricos
    if (phone_number.value.length > 2) {
      phone_number.value = phone_number.value.replace(/(\d{2})(?=\d)/g, "$1-");
    }
  } else {
    phone_number.value = phone_number.value.substring(0, 15);
  }
}
  ...
  if (CV.value) {
    if (photo.value) {
      try {
        ...
        formData.append("photo", photo.value); // Assuming you have an input element for the PDF file
        formData.append("CV", CV.value);
        formData.append("mail", mail.value);
        formData.append("password", password.value);
        formData.append("type_user", "employee");
        ...
        let response = await fetch(`${await Server.GetServer()}/employee`, {
          method: "POST",
          body: formData,
        });
